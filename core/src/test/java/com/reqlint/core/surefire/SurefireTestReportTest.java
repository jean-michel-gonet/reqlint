...
class SurefireTestReportTest {
    ...
    public void can_find_a_test_report_when_its_identifier_is_at_the_begining() {
        ...
        SurefireTestReportItem surefireTestReportItem = new SurefireTestReportItem(
                TIMESTAMP,
                testCaseIdentifier + " - Whatever",
                "output",
                "failure");
        underTest.addReportItem(surefireTestReportItem);

        Assertions.assertThat(underTest.reportsOfTestCase(testCaseIdentifier))
                .contains(surefireTestReportItem);
    }
    ...
    public void can_accept_hyphen_instead_of_underscore() {
        ...
        Assertions.assertThat(underTest.reportsOfTestCase("TC-401")).containsExactly(surefireTestReportItem);
        Assertions.assertThat(underTest.reportsOfTestCase("TC_401")).containsExactly(surefireTestReportItem);
    }
