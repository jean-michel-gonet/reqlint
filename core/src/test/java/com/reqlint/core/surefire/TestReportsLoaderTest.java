...
class TestReportsLoaderTest {
    ...
    public void can_look_for_one_report_by_its_identifier() {
        List<SurefireTestReport> reports = underTest.reportsOfTestCase("TC_101");
    ...
    public void can_look_for_whole_words() {
        List<SurefireTestReport> reports = underTest.reportsOfTestCase("TC_10");
        Assertions.assertThat(reports).isEmpty();
    }

    @Test
    public void can_identify_failed_tests() {
        List<SurefireTestReport> reports = underTest.reportsOfTestCase("TC_102");
        Assertions.assertThat(reports).hasSize(1);
    ...
    public void can_segregate_output_per_stage() {
        List<SurefireTestReport> reports = underTest.reportsOfTestCase("TC_201");
        Assertions.assertThat(reports).hasSize(1);
        SurefireTestReport report = reports.getFirst();
